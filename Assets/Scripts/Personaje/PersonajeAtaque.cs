...
public class PersonajeAtaque : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Time.time > tiempoEntreAtaques)
        {
    ...
    private void UsarArma()
    {
        ...
        {
            if (personajeMana.ManaActual < ArmaEquipada.ManaRequerido)
            {
                return;
            }
            ...
            personajeMana.UsarMana(ArmaEquipada.ManaRequerido);
        }
        else
        {
            EnemigoVida enemigoVida = EnemigoObjetivo.GetComponent<EnemigoVida>();
            enemigoVida.RecibirDano(ObtenerDamage());
        }
    }

    public float ObtenerDamage()
    {
        float cantidad = stats.Dano;
        if (RandomUnity.value < stats.PorcentajeCritico / 100)
        {
            cantidad *= 2;
        }
